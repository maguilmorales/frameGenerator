import { OrbitControls, useFBO, Text, Text3D,  RandomizedLight } from "@react-three/drei";
...
const Geometries = ({rotation, position}) => {
  ...
  return (
    <>
      <mesh ref={mesh} rotation={rotation} position={position}>
...
const Geometries2 = ({rotation, position}) => {
  // This reference gives us direct access to our mesh
  const mesh = useRef();
  const mesh2 = useRef();
  const part = useRef();
...
const CustomGeometryParticles = (props) => {
  ...
  const particlesPosition = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const theta = THREE.MathUtils.randFloatSpread(40); 
      const phi = THREE.MathUtils.randFloatSpread(40); 
  ...
  useFrame((state) => {
    const { clock } = state;
    points.current.rotation.x = 9.5; 
    points.current.rotation.z = 0.3; 
  ...
  return (
    <points ref={points}>
      ...
      <shaderMaterial
        ...
        void main() {
          vec3 color = vec3(0.64, 0.53, 0.96); // Bright Pink
          float strength = distance(gl_PointCoord, vec2(0.5));
          strength = 1.0 - strength;
          strength = pow(strength, 4.0);
        
          color = mix(color, vec3(0.57, 0.20, 0.85), vDistance * 0.9); // Purple
...
const SemiSphere = ({rotation, position}) => {
  

  const meshSemi = useRef();
  const matcapTexture = useLoader(TextureLoader, "matcap4.png");
  const radius = 0.2;
  const widthSegments = 500;
  const heightSegments = 500;
  const phiStart = Math.PI; // Start from PI to create a semisphere
  const phiLength = Math.PI * 2; // Use half of the circle for the semisphere
  ...
  const geometry = useMemo(() => {
    ...
    const bufferGeom = new THREE.BufferGeometry().copy(sphereGeom).toNonIndexed();
    ...
    for (let i = 0; i < numVertices; i++) {
      ...
      if (y < 0) {
        positions[i * 3 + 1] = 0; // Flatten the bottom hemisphere by setting y-coordinate to 0
      }

      // Calculate vertex normals manually
  ...
  const desiredAngle = Math.PI / 8; // Adjust the desired angle to your preference
  const rotationSpeed = 0.5;
  let rotationDirection = 1;

  useFrame((state, delta) => {
    if (meshSemi.current.rotation.z < desiredAngle && rotationDirection === 1) {
      meshSemi.current.rotation.z += delta * rotationSpeed;
    } else if (meshSemi.current.rotation.z > -desiredAngle && rotationDirection === -1) {
      meshSemi.current.rotation.z -= delta * rotationSpeed;
    } else {
  ...
  return (
    <mesh ref={meshSemi} rotation={rotation} position={position} geometry={geometry} material={material} />
  );
};
